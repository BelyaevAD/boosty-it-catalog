...
- фильтрует по теме, активности, цене и росту аудитории;
- относит канал сразу к нескольким подходящим темам и динамически пересчитывает тематические счётчики по соседним фильтрам;
...
- [`data/channels.json`](data/channels.json) — очищенный каталог, который публикуется на сайте; `category` хранит основную тему, а `topics` — все подходящие темы канала;
...
GitHub Actions запускает проверку **каждую пятницу в 17:00 по Москве**:
...
- **Исправить данные** — для неверной тематики, устаревшей или удалённой страницы.
